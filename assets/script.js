/* =========================================================
   프라임포스 — 페이지 동작 (진단 · 카탈로그 탭 · 비용 계산기 · 메뉴)
   단가는 아래 PRICE 한 곳만 고치면 "계산기"와 "진단 예상 월비용"에 반영됩니다.
   카탈로그·업종별 세트에 직접 적힌 금액은 index.html 에서 함께 수정하세요.
   ========================================================= */

// 단가 (원). rent = 월 이용료(기본 요금 기준), buy = 구입가(참고용 예시)
const PRICE = {
  pos:        { name: "포스기",         rent: 22000,  buy: 900000 },
  terminal:   { name: "카드단말기",     rent: 9900,   buy: 150000 },
  tosspay:    { name: "간편결제 단말기", rent: 11000,  buy: 250000 },
...
const RECO = {
  cafe: {
    s: { items: [["terminal",1,"블루투스형 추천"],["pos",1,"선택"]], note: "혼자 운영하는 소형 카페는 단말기만으로 시작해도 충분합니다. 메뉴가 늘면 포스를 붙이세요." },
    m: { items: [["kiosk",1,"탁상형"],["pos",1,"키오스크 연동"],["terminal",1,"백업용"]], note: "피크 시간 주문 대기를 줄이려면 탁상형 키오스크가 가장 효과가 큽니다." },
    l: { items: [["kiosk",2,"스탠드형"],["pos",1,"주방프린터 포함"],["terminal",1,""]], note: "여러 매장을 한 계정에서 보는 포스 구성으로 잡아 드립니다." }
  },
  food: {
    s: { items: [["pos",1,"주방프린터 포함"],["terminal",1,"무선형"]], note: "테이블이 적어도 주방 출력이 있으면 주문 실수가 줄어듭니다." },
    m: { items: [["tableorder",8,"테이블 8개 기준"],["pos",1,"테이블오더 연동"],["terminal",1,"무선형"]], note: "테이블오더는 테이블 수만큼 태블릿이 들어갑니다. 수량은 계산기에서 바꿔 보세요." },
    l: { items: [["tableorder",16,"테이블 16개 기준"],["pos",2,"홀·주방 분리"],["kiosk",1,"포장 주문용"]], note: "대형 식당은 포장 전용 키오스크를 따로 두면 홀 회전이 빨라집니다." }
  },
  unmanned: {
    s: { items: [["kiosk",1,"스탠드형"],["vending",1,"멀티형"]], note: "무인매장의 기본 구성입니다. 매출·재고는 앱으로 원격 확인합니다." },
    m: { items: [["kiosk",1,"스탠드형"],["vending",2,"냉동+멀티"],["pos",1,"관리용"]], note: "품목이 늘면 냉동·상온 자판기를 나누는 편이 관리가 쉽습니다." },
  ...
  office: {
    s: { items: [["vending",1,"음료·스낵"]], note: "장소만 내주고 운영은 맡기는 방식이면 비용 없이 시작할 수도 있습니다. 상담 때 안내드립니다." },
...
function renderFinder(){
  ...
  const cta = document.getElementById("frCta");
  cta.dataset.pick = `진단 결과: ${bizLabel} / ${sizeLabel} → ${parts.join(", ")}`;
  cta.dataset.biz = bizLabel;
}
...
function calc(){
  ...
  if(!any){ v.textContent = "장비를 선택해 주세요."; }
    ...
    const be = rMonth ? Math.ceil(bInit / rMonth) : 0;
...
document.addEventListener("click", e => {
  ...
  if(a.dataset.biz){
    ...
    [...sel.options].forEach(o => { if(o.value && a.dataset.biz.startsWith(o.value.split("·")[0])) sel.value = o.value; });
  }
});
